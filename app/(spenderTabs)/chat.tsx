...
export default function ChatCoachScreen() {
  ...
  const fetchCoachData = async () => {
    try {
      ...
      if (userError || !user) throw userError || new Error('No authenticated spender account found.');
      ...
      const spenderName = profileData?.full_name || 'Spender';
      ...
      const allowanceId = allowanceData?.id ?? null;
      const allowanceAmount = Number(allowanceData?.amount || 0);
      ...
      if (allowanceId) {
        budgetsQuery = budgetsQuery.eq('allowance_id', allowanceId);
      }
      ...
      const categoryBreakdown = Array.from(categoryMap.entries())
        .map(([name, amount], index) => ({
          name,
          amount,
          percent: allowanceAmount > 0 ? (amount / Math.max(allowanceAmount, 1)) * 100 : 0,
      ...
      const unallocated = Math.max(allowanceAmount - totalAllocated, 0);
      const safeToSpend = remaining;
      const healthScore = allowanceAmount > 0 ? clamp(Math.round((remaining / allowanceAmount) * 100), 0, 100) : 0;
      ...
      >
        {loading ? (
          <View style={styles.loadingState}>
            ...
            <Text style={styles.loadingText}>Loading spender insights...</Text>
          </View>
...
function Predictions({ summary }: { summary: CoachSummary }) {
  const usageRate = summary.allowance > 0 ? (summary.totalSpent / summary.allowance) * 100 : 0;
  ...
  const unusualText =
    summary.lastWeekTotal > 0 && weekIncreasePct > 10
      ...
      : summary.thisWeekTotal > summary.lastWeekTotal
        ...
        : `Your spending is staying under last week’s pace, which is positive for your remaining allowance and monthly plan.`;

  const behaviorText =
    weekendVsWeekday > 0
      ? `Weekend spending is currently ${toMoney(Math.abs(weekendVsWeekday))} higher than weekday spending. If this pattern keeps going, your remaining allowance could tighten before the month ends.`
      : `Your weekday spending is currently heavier than weekend spending, which suggests your spend is more routine-driven. Protecting your ${summary.topCategory.toLowerCase()} category will keep your allowance stable.`;
      ...
      <View style={styles.safeCard}>
        ...
        <View style={styles.gaugeContainer}>
          <View style={styles.gaugeText}>
            ...
            <Text style={styles.limitText}>LIMIT: {toMoney(summary.allowance)}</Text>
          </View>
...
function Insights({ summary }: { summary: CoachSummary }) {
  ...
  const weekendText = summary.weekendSpending >= summary.weekdaySpending
    ...
    : `Your spending is relatively stable across weekdays and weekends. Keep the pattern balanced to preserve your remaining allowance.`;
        ...
        <View style={styles.featureContent}>
          ...
          <Text style={styles.featureText}>
            {summary.reminder
              ...
              : 'No pending recurring fees are logged for this spender account yet.'}
          </Text>
        </View>
      </View>
