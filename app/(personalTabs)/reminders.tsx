...
export default function RemindersScreen() {
  ...
  const fetchRemindersAndCategories = async () => {
    try {
      ...
      const { data: remData, error: remError } = await supabase
        .from('reminders')
        .select(`
          id, title, amount, category_id, income_id, due_date, status,
  ...
  const handleSaveReminder = async () => {
    ...
    try {
      ...
      const { data: activeIncome } = await supabase
        .from('income')
        .select('id')
        .eq('user_id', user.id)
      ...
      const { error } = await supabase.from('reminders').insert({
        ...
        income_id: activeIncome?.id || null,
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              let activeIncomeId = reminder.income_id;
              ...
              if (!activeIncomeId) {
                ...
                activeIncomeId = activeIncome?.id || undefined;
              }

              // Fetch budget id and total allocated amount based on category and income
              let budgetQuery = supabase
                .from('budgets')
                .select('id, income_id, allocated_amount') // Fetch allocated budget amount
                .eq('user_id', user.id)
                .eq('category_id', reminder.category_id);

              if (activeIncomeId) {
                budgetQuery = budgetQuery.eq('income_id', activeIncomeId);
              }
              ...
              const { error: expenseError } = await supabase
                ...
                .insert({
                  budget_id: budget.id,
                  income_id: activeIncomeId || budget.income_id,
